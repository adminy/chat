...
export default {
  ...
  methods: {
    resetData () {
      roomsRef.get().then(async rooms => {
        for (const room of rooms) {
          const ref = roomsRef.doc(room.id).collection('messages')
          const res = await ref.get()
          !res.empty && res.docs.map(doc => ref.doc(doc.id).delete())
          roomsRef.doc(val.id).delete()
        }
      })

      usersRef.get().then(users => users.forEach(user => usersRef.doc(user.id).delete()))
    },
    async addData () {
      ...
      await roomsRef.add({ users: [user1._id, user2._id], lastUpdated: new Date() })
      await roomsRef.add({ users: [user1._id, user3._id], lastUpdated: new Date() })
      await roomsRef.add({ users: [user2._id, user3._id], lastUpdated: new Date() })
      await roomsRef.add({ users: [user1._id, user2._id, user3._id], lastUpdated: new Date() })
...
export default {
	...
	methods: {
		...
		async editMessage({ messageId, newContent, roomId, file }) {
			file = file || deleteDbField
			const newMessage = {
				edited: new Date(),
				content: newContent,
				file
			}
			newMessage.file.extension = newMessage.file.extension || newMessage.file.type
			newMessage.file.url = newMessage.file.url || newMessage.file.localUrl
			await messagesRef(roomId).doc(messageId).update(newMessage)
			file && file.blob && this.uploadFile({ file, messageId, roomId })
		},
